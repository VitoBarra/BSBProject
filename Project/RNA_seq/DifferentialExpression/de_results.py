...
def _cluster_order(matrix: np.ndarray) -> list[int]:
    """Return a complete-linkage clustering order without extra dependencies."""
    ...
    while len(clusters) > 1:
        ...
        for left in range(len(clusters) - 1):
            for right in range(left + 1, len(clusters)):
                complete = float(np.max(distances[np.ix_(clusters[left], clusters[right])]))
...
def _plot_top_gene_heatmap(
    ...
    row_order = _cluster_order(row_z)
    column_order = _cluster_order(row_z.T)
    ...
    figure, ax = plt.subplots(figsize=(7.5, 8))
    ...
    ax.set_title("Top 20 differentially expressed genes (row z-scores)")
    figure.colorbar(image, ax=ax, label="Row z-score")
...
def generate_de_outputs(results_path: Path, normalized_counts_path: Path, vst_counts_path: Path, samples_path: Path) -> None:
    ...
    _plot_top_gene_heatmap(results, vst_rows, vst_matrix, sample_names, output_dir)
